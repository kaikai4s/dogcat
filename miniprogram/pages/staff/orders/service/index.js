...
Page({
  ...
  uploadPoint() { getServiceLocation().then((loc) => { callFunction('track', 'batchUploadTrack', { orderId: this.data.id, points: [{ latitude: loc.latitude, longitude: loc.longitude, accuracy: loc.accuracy, speed: 0, recordedAt: new Date() }] }).then((res) => this.setData({ pointCount: this.data.pointCount + res.count })).catch(showError) }).catch(showError) },
