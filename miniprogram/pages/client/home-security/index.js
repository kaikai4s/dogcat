...
Page({
  ...
  onShow() {
    ensureLogin({ content: '登录后可管理家庭安防信息。' })
      .then(() => this.load())
      .catch(() => wx.redirectTo({ url: '/pages/client/home/index' }))
  },
  load() { callFunction('homeSecurity', 'getMaskedHomeSecurity').then((data) => { if (data) this.setData({ form: { ...this.data.form, ...data, doorLockCode: '' }, masked: data.doorLockCodeMasked || '' }) }).catch(showError) },
