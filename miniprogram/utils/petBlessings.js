...
function resolvePetBlessing(prize = {}) {
  ...
  if (name) {
    ...
    if (!matched) {
      matched = PET_BLESSINGS.find((b) => name.includes(b.breed) || b.name.includes(name) || b.breed.includes(name))
    }
  }

  const snapshot = prize.snapshot || prize.prizeSnapshot
  if (!matched && snapshot) {
    const snapBreed = String(snapshot.breed || '').trim()
    const snapName = String(snapshot.name || '').trim()
    if (snapBreed) {
      matched = PET_BLESSINGS.find((b) => b.breed === snapBreed || b.breed.includes(snapBreed))
    }
    if (!matched && snapName) {
      matched = PET_BLESSINGS.find((b) => b.name === snapName || snapName.includes(b.breed))
    }
    if (!matched && snapshot.text && snapshot.text !== snapshot.name) {
      return {
        name: name || snapName || '萌宠祝福',
  ...
  if (matched) {
    ...
    return {
      ...
      text: useMatchedText ? matched.text : text
    }
  }
