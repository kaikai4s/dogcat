...
> 实施状态（2026-09-21）：25 个真实 handler、生产路由、常量、基础工具与定时编排已完成迁移；共享业务已进一步拆为 53 个领域服务，context.js 仅负责装配。已增加提现事务、服务订单超时查单/关单及接单派单并发控制。当前结构、发布步骤和剩余风险以 [ARCHITECTURE.md](./ARCHITECTURE.md) 为准。下文为历史设计草案，其中工期、行数和性能提升比例不是实测结果；其他资金与订单一致性风险仍需逐项审查。
