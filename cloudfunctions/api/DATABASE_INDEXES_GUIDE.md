...
### 索引 1.3：订单状态与时间窗口复合查询（防全表扫描）
**优先级**：⭐⭐⭐ P0（最高优先级，核心性能保障）

| 配置项 | 填写内容 |
|--------|----------|
| 索引名称 | `idx_status_start_time` |
| 索引属性 | 非唯一 |
| 字段 1 | `status` - 升序 |
| 字段 2 | `startTime` - 升序 |

**用途**：
- **定时任务性能优化**：供超时巡检任务（超时 15/30 分钟未开始催办预警、超时 30 分钟未结束智能自动完单结算、待接单超时自动全额退款）使用。
- **防止全表扫描**：所有后台定时器强制携带活跃时间窗口条件（`startTime: _.gte(7天前)`），结合此复合索引直接定位活跃数据范围，彻底杜绝随着历史订单累积导致的云数据库全表扫描与配额耗尽。
