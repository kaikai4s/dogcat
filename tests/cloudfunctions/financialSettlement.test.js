  ...
  db.runTransaction = async callback => {
    for (let attempt = 0; attempt < 8; attempt++) {
      ...
      const result = await callback({ collection(name) { return { doc(id) {
        const remember = () => reads.set(`${name}/${id}`, { name, id, value: JSON.stringify(snapshot.state[name]?.find(row => row._id === id)) })
        return {
      ...
      if ([...reads.values()].some(({ name, id, value }) => JSON.stringify(db.state[name]?.find(row => row._id === id)) !== value)) {
        retries++; continue
      }
