...
test('real-device acceptance core flow covers client staff admin lifecycle', async () => {
  ...
  assert.equal(report.data.order.orderHomeSecurity, undefined)
  assert.equal(report.data.order.homeSecuritySnapshot, undefined)
  assert.equal(report.data.order.serviceAddress, undefined)
  assert.equal(report.data.order.addressLatitude, undefined)
  assert.equal(report.data.order.contactPhone, undefined)
  assert.equal(review.ok, true)
...
test('admin can save rich member level and coupon template settings', async () => {
  ...
  const level = await fn.main({ module: 'admin', action: 'saveMemberLevel', data: { name: '钻石会员', badgeTag: 'VIP-DIAMOND', nameColor: '#67c23a', nameEffect: 'emerald_glow', badgeStyle: 'green', minPoints: 300, pointMultiplier: 3, description: '高阶会员', benefits: ['专属券', '高倍积分'] } })
  const levels = await fn.main({ module: 'admin', action: 'listMemberLevels', data: {} })
  ...
  assert.equal(level.data.nameColor, '#67c23a')
  assert.equal(level.data.nameEffect, 'emerald_glow')
  assert.equal(level.data.badgeStyle, 'green')
  assert.equal(level.data.pointMultiplier, 3)
  assert.deepEqual(level.data.benefits, ['专属券', '高倍积分'])
  assert.equal(levels.ok, true)
  assert.equal(levels.data[0].nameEffect, 'emerald_glow')
  assert.equal(levels.data[0].badgeStyle, 'green')
